                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select name="estado" class="form-select" required>
                            <option value="activo" {{ old('estado', $servicio->estado) == 'activo' ? 'selected' : '' }}>
                                Activo</option>
                    ...
                    <div class="form-group" style="grid-column:1/-1;">
                        ...
                        @if ($servicio->imagen)
                            <img src="{{ asset('storage/' . $servicio->imagen) }}" alt="{{ $servicio->nombre }}"
                                style="width:80px;height:80px;border-radius:10px;object-fit:cover;border:2px solid var(--sb-accent-soft);margin-bottom:0.5rem;">
                            <p style="font-size:0.8rem;color:var(--sb-muted);margin-bottom:0.5rem;">
                                {{ basename($servicio->imagen) }}</p>
                        @else
                            <p style="font-size:0.8rem;color:var(--sb-muted);margin-bottom:0.5rem;">Sin imagen</p>
                        @endif
                        <label class="form-label">Nueva Imagen (opcional)</label>
                        <div class="form-file-wrapper">
                            <input type="file" name="imagen" id="imagenInput" accept="image/jpeg,image/png,image/webp">
                            <p id="fileName">📷 Dejar vacío para mantener la imagen actual</p>
                        </div>
                        @error('imagen')
                            <span class="error-msg">{{ $message }}</span>
                        @enderror
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (input && name) {
                input.addEventListener('change', () => {
                    ...
                    else name.textContent = '📷 Dejar vacío para mantener la imagen actual';
                });
            }
        });
    </script>
