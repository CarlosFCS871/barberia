...
@section('BarberoContenido')
    <style>
        ...
        .perfil-form .form-group:nth-child(1) {
            animation-delay: 0.2s
        }

        .perfil-form .form-group:nth-child(2) {
            animation-delay: 0.25s
        }

        .perfil-form .form-group:nth-child(3) {
            animation-delay: 0.3s
        }

        .perfil-form .form-group:nth-child(4) {
            animation-delay: 0.35s
        }

        .perfil-form .form-group:nth-child(5) {
            animation-delay: 0.4s
        }

        .perfil-form .form-group:nth-child(6) {
    ...
    <div class="perfil-container">
        <header class="perfil-header">
            <h1>💈 Mi Perfil de Barbero</h1>
            <p>Administra tu información personal y seguridad de cuenta</p>
        </header>
                ...
                <div class="perfil-avatar">
                    @if ($user->foto)
                        <img src="{{ asset('storage/' . $user->foto) }}" alt="Foto Perfil"
                            onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($user->nombre ?? 'B') }}&background=F5DC5B&color=000&size=240'">
                    @else
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($user->nombre ?? 'B') }}&background=F5DC5B&color=000&size=240"
                            alt="Avatar">
                    @endif
                </div>
                <span class="avatar-label">Foto de perfil</span>
                <span class="estado-badge estado-{{ $user->estado }}">{{ ucfirst($user->estado) }}</span>
            </div>

            {{-- FORMULARIO - LOGICA INTACTA 100% ================= --}}
            <form action="{{ route('barbero.perfil.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                @csrf @method('PUT')
                <div class="perfil-form">
                    <div class="form-group">
                        <label class="form-label">Nombre</label>
                        <input type="text" name="nombre" class="form-input" value="{{ old('nombre', $user->nombre) }}"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="apellido" class="form-input"
                            value="{{ old('apellido', $user->apellido) }}" placeholder="Apellido" required>
                        @error('apellido')
                            <span class="error-msg">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="telefono" class="form-input"
                            value="{{ old('telefono', $user->telefono) }}" placeholder="Teléfono">
                        @error('telefono')
                            <span class="error-msg">{{ $message }}</span>
                        @enderror
                    </div>
    ...
    {{-- Toast Notification (optional, triggered by backend session) --}}
    @if (session('success'))
